...
export const schemas = {
  ...
  // Demographics
  gender: z.enum(['M', 'F', 'U'], { message: 'Gender must be M, F, or U' }),
...
export type PatientStatus = z.infer<typeof schemas.patientStatus>;
export type AlarmStatus = z.infer<typeof schemas.alarmStatus>;
export type AlarmType = z.infer<typeof schemas.alarmType>;

// Alarm limits type
const alarmLimitsBaseSchema = z.object({
...
export const createPatientSchema = z.object({
  ...
  alarmLimits: alarmLimitsBaseSchema.optional().nullable(),
});
...
export const alarmActionSchema = z.object({
  action: z.enum(['acknowledge', 'silence', 'resolve'], {
    message: 'Action must be acknowledge, silence, or resolve'
  }),
...
export type AlarmActionInput = z.infer<typeof alarmActionSchema>;
...
export const createDeviceSchema = z.object({
  ...
  type: z.enum(deviceTypes, { message: 'Invalid device type' }),
...
export const deviceLogLevels = ['info', 'warning', 'error', 'critical'] as const;
export type DeviceLogLevel = typeof deviceLogLevels[number];
...
export type DeviceLogCategory = typeof deviceLogCategories[number];

export const createDeviceLogSchema = z.object({
  level: z.enum(deviceLogLevels).default('info'),
  category: z.enum(deviceLogCategories, { message: 'Invalid log category' }),
  message: z.string()
...
export type CreateDeviceLogInput = z.infer<typeof createDeviceLogSchema>;
...
export type AlarmLimits = z.infer<typeof alarmLimitSchema>;
...
export type UpdateAlarmLimitsInput = z.infer<typeof updateAlarmLimitsSchema>;
...
export type AlarmPresetName = typeof alarmPresetNames[number];
...
export const createOrderSchema = z.object({
  patientId: z.number().int().positive('Patient ID is required'),
  category: z.enum(orderCategories, { message: 'Invalid order category' }),
  orderType: z.enum(orderTypes, { message: 'Invalid order type' }),
...
export const carePlanCategories = [
  // Uppercase Prisma enum values
  'RESPIRATORY', 'NUTRITION', 'NEUROLOGICAL', 'INFECTION', 'GROWTH_DEVELOPMENT',
  'SKIN_WOUND', 'FAMILY_SUPPORT', 'PAIN_MANAGEMENT', 'DEVELOPMENTAL', 'DISCHARGE_PLANNING', 'OTHER',
  // Lowercase for backwards compatibility
  'respiratory', 'nutrition', 'neuro', 'neurological', 'infection', 'growth', 'growth_development',
  'skin', 'skin_wound', 'family', 'family_support', 'pain', 'pain_management', 'developmental',
  'discharge', 'discharge_planning', 'other',
] as const;

export type CarePlanCategory = typeof carePlanCategories[number];

// Uppercase to match Prisma enums
export const carePlanPriorities = ['HIGH', 'MEDIUM', 'LOW', 'high', 'medium', 'low'] as const;
export type CarePlanPriority = typeof carePlanPriorities[number];

export const carePlanStatuses = ['ACTIVE', 'ON_HOLD', 'COMPLETED', 'DISCONTINUED', 'ARCHIVED', 'active', 'on_hold', 'completed', 'discontinued', 'archived'] as const;
export type CarePlanStatus = typeof carePlanStatuses[number];

export const carePlanItemTypes = ['TASK', 'ASSESSMENT', 'INTERVENTION', 'EDUCATION', 'MONITORING', 'CONSULTATION', 'task', 'assessment', 'intervention', 'education', 'monitoring', 'consultation'] as const;
export type CarePlanItemType = typeof carePlanItemTypes[number];

export const carePlanItemStatuses = ['PENDING', 'IN_PROGRESS', 'COMPLETED', 'SKIPPED', 'DEFERRED', 'pending', 'in_progress', 'completed', 'skipped', 'deferred'] as const;
export type CarePlanItemStatus = typeof carePlanItemStatuses[number];
...
export const createCarePlanSchema = z.object({
  ...
  category: z.enum(carePlanCategories, { message: 'Invalid care plan category' }),
...
export type UpdateCarePlanItemInput = z.infer<typeof updateCarePlanItemSchema>;

// =====================================================
// DISCHARGE SCHEMAS
// =====================================================

export const dischargeDispositions = ['home', 'transfer', 'hospice', 'deceased'] as const;
export type DischargeDisposition = typeof dischargeDispositions[number];

export const dischargeStatuses = ['planning', 'ready', 'pending_approval', 'discharged', 'cancelled'] as const;
export type DischargeStatus = typeof dischargeStatuses[number];

export const checklistCategories = ['medical', 'equipment', 'education', 'followup', 'documentation', 'safety'] as const;
export type ChecklistCategory = typeof checklistCategories[number];

export const checklistItemStatuses = ['pending', 'in_progress', 'completed', 'not_applicable'] as const;
...
export type CreateDischargePlanInput = z.infer<typeof createDischargePlanSchema>;
...
export type UpdateDischargePlanInput = z.infer<typeof updateDischargePlanSchema>;
...
export const handoffStatuses = ['draft', 'submitted', 'acknowledged'] as const;
export type HandoffStatus = typeof handoffStatuses[number];

export const acuityLevels = ['stable', 'moderate', 'critical'] as const;
export type AcuityLevel = typeof acuityLevels[number];

export const createHandoffNoteSchema = z.object({
  patientId: z.number().int().positive('Patient ID is required'),
  shift: z.enum(shiftTypes, { message: 'Invalid shift type' }),
...
export const growthMeasurementInputSchema = z.object({
  ...
  data => {
    ...
    const hasHC = data.hc !== null && data.hc !== undefined;
    return hasWeight || hasLength || hasHC;
  },
  { message: 'At least one measurement is required' }
);

export type GrowthMeasurementInputType = z.infer<typeof growthMeasurementInputSchema>;

// =====================================================
// FAMILY PORTAL SCHEMAS
// =====================================================

export const messageTypes = ['general', 'update', 'question', 'response', 'alert', 'education'] as const;
export type MessageType = typeof messageTypes[number];

export const messageStatuses = ['pending', 'sent', 'delivered', 'read', 'failed'] as const;
export type MessageStatus = typeof messageStatuses[number];

export const messageChannels = ['app', 'email', 'sms'] as const;
...
export type CreateFamilyMessageInput = z.infer<typeof createFamilyMessageSchema>;
...
export const educationContentTypes = ['article', 'video', 'checklist', 'faq', 'pdf'] as const;
export type EducationContentType = typeof educationContentTypes[number];

// Zod issue type for compatibility
interface ZodIssue {
  path: (string | number)[];
  message: string;
}
...
export function validateRequest<T>(schema: z.ZodSchema<T>, data: unknown): ValidationResult<T> {
  ...
  if (!result.success) {
    ...
    const errors: ValidationErrorDetail[] = issues.map(err => ({
      field: err.path.join('.'),
      message: err.message,
    }));
...
export type ValidatorFunction<T> = (data: unknown) => Promise<T>;

// Middleware-style validator for API routes
export function createValidator<T>(schema: z.ZodSchema<T>): ValidatorFunction<T> {
